...
- Se suelen inyectar servicios como `ElementRef` y `Renderer2` para manipular el DOM de manera segura.
  - `ElementRef`: Permite acceder al elemento del DOM al que está asociada la directiva, que se encuentra en la propiedad `nativeElement`.
...
El elemento Host es el elemento del DOM al que está asociada la directiva. En un componente, el host es el propio componente.
...
- la directiva queda vinculada al eleemnto al que se aplicara (el.nativeElement es un HTMLDivElement)
...
En las directivas estructurales, la inyección de dependencias nos poprporciona dos servicios clave:
